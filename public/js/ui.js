...
function createAchievementCard(name, description, iconName) {
...
export function addXPVisual(amount, x, y) {
    if (x && y) {
...
export async function renderOrders() {
    ...
    const panel = scr.firstElementChild;
    if (panel) {
...
export function renderSummary() {
    ...
    if (elTotal)
        elTotal.innerText = formatCurrency(AppState.sessionTotal);
    const elXP = document.getElementById('summary-xp');
    if (elXP)
        elXP.innerText = `+${AppState.sessionXP} XP`;
