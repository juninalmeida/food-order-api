...
function resolveIconName(iconName) {
    if (!iconName)
        return DEFAULT_ICON;
    if (SOLAR_ICONS[iconName])
        return iconName;
    const alias = ICON_ALIASES[iconName];
    if (alias && SOLAR_ICONS[alias])
        return alias;
    return DEFAULT_ICON;
}
function buildTransform(icon) {
    const transforms = [];
    ...
    if (icon.hFlip)
        transforms.push(`translate(${left + width * 2} 0) scale(-1 1)`);
    if (icon.vFlip)
        transforms.push(`translate(0 ${top + height * 2}) scale(1 -1)`);
    if (icon.rotate)
        transforms.push(`rotate(${icon.rotate * 90} ${centerX} ${centerY})`);
