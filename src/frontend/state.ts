import { OrderItem, Product, Achievement, LevelInfo } from "./types.js";
...
class State {
    ...
    getLevelInfo(): LevelInfo {
        if(this.xp >= 1000) return { name: 'Lenda da Cozinha', badge: 'solar:crown-linear', color: 'rainbow-text', min: 1000, max: 2000 };
        if(this.xp >= 500) return { name: 'Mestre Chef', badge: 'solar:star-fall-linear', color: 'text-[#eab308]', min: 500, max: 1000 };
        if(this.xp >= 250) return { name: 'Chef', badge: 'solar:chef-hat-linear', color: 'text-[#94a3b8]', min: 250, max: 500 };
        if(this.xp >= 100) return { name: 'Cozinheiro', badge: 'solar:fire-linear', color: 'text-[#b45309]', min: 100, max: 250 };
