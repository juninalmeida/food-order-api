...
class TablesService {
  ...
  async openAndResolveSessionId(tableId: number) {
  ...
  getActiveSessions(sessions: TableSession[]) {
    return sessions.filter((session) => !session.closed_at);
  }

  findSessionForTable(tableId: number, activeSessions: TableSession[]) {
    return activeSessions.find((session) => session.table_id === tableId);
  }

  findPreferredTable(tables: Table[], tableNumber: number) {
